...
import { T18_IMAGE } from "@assets";
type TeethMapProps = {
  tooth: FiveZoneTooth;
  onColorChange: (tooth: keyof FiveZonesChart, segment: keyof Segment4) => void;
};

export const T18Svg: FC<TeethMapProps> = ({ tooth, onColorChange }) => {
  ...
  const className = "tooth-svg l u 8";
  if (tooth.removed) {
    return (
  ...
  return (
    <SvgIcon className="tooth-svg l u big 8">
      <svg width="30.08" height="58.24" version="1.1" viewBox="0 0 30.08 58.24">
        <T18_IMAGE />
        <g fillOpacity=".65229">
          <path
            onClick={() => onColorChange("t18", "c4")}
            style={style_4}
