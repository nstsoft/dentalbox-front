
import React, { Component } from 'react';
import * as actions from '../../store/actions/index';
import { connect } from 'react-redux';
import * as R from 'ramda';

import implant from '../../images/teeth-icons/implant.png';
import crown from '../../images/teeth-icons/crown.png';
import removed from '../../images/teeth-icons/removed.png'

class T41 extends Component {

    constructor ( props ) {
        super( props );
        this.colorSet = [ 'none', 'rgb(102, 89, 89)', 'rgb(176, 71, 71)', 'rgb(51, 87, 163)', 'rgb(57, 141, 40)' ];

    }

    changeColor = ( event ) => {
        const { isToothToggleable } = this.props;
        if (!isToothToggleable) return;
        
        let segment = event.target;
        let code    = segment.attributes.seg.value;
        let indexOf = this.colorSet.indexOf( segment.style.fill ) + 1;

        if ( indexOf === 5 ) {

            indexOf = 0;

        }

        let newColor = this.colorSet[ indexOf ];

        let teeth = {

            '41': {

                [code]: { color: newColor },
                removed: false,
                implant: false

            }

        };

        this.props.setTeethChart( teeth, '41' )

    };

    render () {

        if( R.path(['tooth', '41','implant'], this.props) ){return <img  src={implant} />}
        if( R.path(['tooth', '41','crown'], this.props)){return <img  src={crown} />}
        if( R.path(['tooth', '41','removed'], this.props)){return <img   src={removed} />}

        let style_1 = {
            'fill': R.path( ['tooth', '41', '41-1', 'color'], this.props ) || 'none',
            'stroke': '#000000',
            'strokeWidth': '0.26418332px',
            'strokeLinecap': 'butt',
            'strokeLinejoin': 'miter',
            'strokeOpacity': '0'
        };
        let style_2 = {
            'fill': R.path( ['tooth', '41', '41-2', 'color'], this.props ) || 'none',
            'stroke': '#000000',
            'strokeWidth': '0.26418332px',
            'strokeLinecap': 'butt',
            'strokeLinejoin': 'miter',
            'strokeOpacity': '0'
        };
        return (
        <svg
            viewBox="0 0 210 297"
            version="1.1">
            <g>
            <image
                y="-293.16449"
                x="52.848682"
                id="image6606"
                xlinkHref="data:image/png;base64,
                "
                    preserveAspectRatio="none"
                    height="283.14395"
                    width="79.860992"
                    transform="scale(1,-1)"/>
                <path
                    seg="41-1"
                    onClick={this.changeColor}
                    style={style_1}
                    d="m 89.826129,290.3011 c 0.381496,-0.74817 0.650043,-1.55381 0.79375,-2.38125 0.371304,-2.13789 -0.09335,-4.31917 -0.264584,-6.4823 -0.160162,-2.02328 -0.06232,-4.05676 0,-6.08541 0.169793,-5.52674 0.07829,-11.09466 1.058334,-16.53646 0.619314,-3.4388 1.661523,-6.79087 2.910416,-10.05417 1.823752,-4.76538 4.090127,-9.35525 5.820825,-14.15521 1.26815,-3.51712 2.24308,-7.13151 3.30729,-10.71562 1.8193,-6.12716 3.90398,-12.18036 5.42396,-18.38854 0.90747,-3.70645 1.61193,-7.46239 2.11667,-11.24479 1.09868,-8.2333 1.24862,-16.56673 1.05833,-24.87083 -0.29836,-13.02044 -1.35342,-26.51934 3.43958,-38.62917 0.82798,-2.09194 1.82985,-4.13762 2.27372,-6.34324 0.20309,-1.00918 0.28753,-2.04486 0.58719,-3.02969 0.12439,-0.40879 0.28647,-0.81227 0.31907,-1.23832 0.0163,-0.21302 -9.2e-4,-0.43056 -0.0704,-0.63259 -0.0695,-0.20203 -0.19335,-0.38807 -0.36559,-0.51448 -0.16715,-0.12267 -0.37314,-0.18498 -0.57963,-0.20354 -0.2065,-0.0186 -0.41461,0.005 -0.61892,0.0399 -0.69316,0.11972 -1.35394,0.3779 -2.03459,0.55542 -1.22034,0.31828 -2.4909,0.37434 -3.74378,0.51867 -1.70307,0.19619 -3.38303,0.55727 -5.08188,0.7871 -4.10946,0.55596 -8.277948,0.33882 -12.424057,0.25794 -2.905402,-0.0567 -5.813014,-0.0459 -8.715206,-0.19374 -3.776722,-0.19237 -7.558717,-0.65746 -11.18529,-1.72919 -1.352217,-0.39961 -2.678303,-0.88215 -4.00085,-1.37107 -0.107482,-0.0397 -0.214956,-0.0795 -0.32317,-0.11721 -0.04233,-0.0147 -0.08532,-0.0293 -0.130012,-0.0328 -0.04469,-0.003 -0.09193,0.005 -0.127238,0.0328 -0.02128,0.0166 -0.03731,0.0395 -0.04773,0.0644 -0.01042,0.0249 -0.01539,0.0519 -0.01669,0.0789 -0.0026,0.054 0.0091,0.10741 0.01762,0.16076 0.02087,0.13123 0.02216,0.26469 0.0234,0.39757 0.05103,5.45087 0.10311,10.90298 0.33922,16.34897 0.329088,7.5906 1.015392,15.16266 1.32292,22.75416 0.939153,23.18351 -1.658089,46.399 -0.79375,69.58542 0.394618,10.58586 1.510252,21.18105 0.79375,31.75 -0.340994,5.02992 -1.094208,10.10852 -0.26458,15.08125 0.271648,1.62824 0.712225,3.22887 1.32291,4.7625 1.717799,4.31396 4.773436,8.03346 8.46667,10.84791 0.886219,0.67535 1.81627,1.30532 2.836201,1.7539 1.019931,0.44857 2.138148,0.71103 3.249218,0.62735 1.224388,-0.0922 2.412104,-0.6148 3.307291,-1.4552 z"
                    id="41-1"
                />
                <path
                    seg="41-2"
                    onClick={this.changeColor}
                    style={style_2}
                    d="m 113.87013,119.28104 v 0 l -1.81901,0.33073 -2.11667,0.33073 -3.77031,0.59532 -7.044522,0.26458 -15.490901,-0.3125 -8.65285,-1.26285 -4.49013,-1.44993 -1.1693,-0.46773 -0.11693,-0.0468 -0.0468,-0.0234 -0.0117,-0.18709 -0.47942,-1.56686 -0.37418,-2.05798 -0.28063,-2.43215 -1.40316,-11.038232 -3.55469,-8.98026 -8.8093,-23.05785 -1.5875,-6.87917 1.32292,-19.57917 3.43958,-13.49375 3.70417,-8.46666 11.90625,-8.20209 18.520838,-1.8520796 21.166665,2.9104196 7.9375,9.525 10.31875,15.875 2.11666,11.64166 -3.175,10.31875 -4.23333,15.34584 -2.38565,12.35942 -2.80633,15.341272 -1.6838,11.03823 v 2.99342 l -0.14032,1.02899 -0.23386,0.46772 -0.65481,0.0468 -2.45209,0.68881 -1.45018,0.25386 z"
                    id="41-2"
                />
            </g>
        </svg> )
    }
}

const mapStateToProps = state => {

    return state;

};

const mapDispatchToProps = dispatch => {

    return {

        setTeethChart: ( data, type ) => dispatch( actions.setTeethChart( data, type ) ),

    };
    
};

export default connect( mapStateToProps, mapDispatchToProps )( T41 );








